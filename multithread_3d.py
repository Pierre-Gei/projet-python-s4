...
def save(data, file_name):
    ...
    writer.writerow(["x", "y","z", "centroid x", "centroid y","centroid z"])
    for i in range(len(data)):
...
def get_best_data(best_thread_number):
    ...
    file.close()
    for i in range(len(data)):
        data[i] = [float(data[i][0]), float(data[i][1]), float(data[i][2]), float(data[i][3]), float(data[i][4]), float(data[i][5])]
    return data
...
def kmeans(data, clusters_number, thread_number):
    ...
    cpt = 0
    while mean_distance != old_mean_distance and cpt < 20:
        cpt += 1
...
if __name__ == "__main__":
    ...
    dataset = getDataset('./data/3d_data.csv')
    run(dataset, 10, 5)